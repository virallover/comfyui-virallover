...
class DeHaloAlphaWithMaskTorch:
    @staticmethod
    ...
    RETURN_TYPES = ("IMAGE",)
    FUNCTION = "dehalo_with_mask"
    CATEGORY = "image/fix"

    def dehalo_with_mask(self, image, mask, low_sens, high_sens, darkstr, brightstr):
        # 记录输入格式
        ...
        img = img_chw[0]  # [3, H, W]
        msk = mask_chw[0, 0]  # [H, W]

        # Downscale and upscale (blur effect)
        img_ = img.unsqueeze(0)  # [1, 3, H, W]
        img_down = F.interpolate(img_, scale_factor=0.5, mode='bicubic', align_corners=False)
        img_blur = F.interpolate(img_down, size=img.shape[1:], mode='bilinear', align_corners=False).squeeze(0)  # [3, H, W]

        # Compute difference mask
        diff = torch.abs(img - img_blur)
        diff_gray = 0.2989 * diff[0] + 0.5870 * diff[1] + 0.1140 * diff[2]  # [H, W]
        ...
        # Apply external mask
        final_mask = halo_mask * msk  # [H, W]

        # Merge blur and original
        merged = img * (1 - final_mask) + img_blur * final_mask  # [3, H, W]

        # Recovery
        correction_factor = torch.where(img < merged, darkstr, brightstr)
        corrected = img - (img - merged) * correction_factor  # [3, H, W]

        # 输出格式还原
        out = corrected.unsqueeze(0)  # [1, 3, H, W]
        if input_is_nhwc:
            out = out.permute(0, 2, 3, 1).contiguous()  # [1, H, W, 3]
        out = out.clamp(0, 1).to(torch.float32)
        assert out.shape == image.shape, f"输出shape错误: {out.shape} vs {image.shape}"
        return (out,)
